...
function createGradientConfig(): GradientConfig {
    return {
        y2: `${93 + Math.random() * 8}%`,
...
const BackgroundBeams: FC<{ className?: string }> = ({ className }) => {
    // Pre-compute the random animation targets/timings once per mount so the
    // render stays pure (no Math.random during render).
    const gradientConfigs = React.useMemo(
        () => paths.map(() => createGradientConfig()),
        [],
    );
    return (
        <div
