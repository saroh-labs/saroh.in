...
import { LibraryPanel } from "./photo-library-panel";
...
export function PhotosSection({ product }: { product: ProductDetail | null }) {
    ...
    const limit = mediaLimitNote(draft);
    ...
    function takeOff(i: number) {
        ...
        const name = photo.alt || fileName(photo);
        showUndo(`${name} taken off. It stays in your photos.`, () =>
            setDraft(before),
        );
    }
        ...
        <SectionCard k="photos" title="Photos and videos">
            <div className="mb-2.5 flex flex-wrap items-baseline gap-2.5">
                ...
                <span className="flex-[1_1_200px] text-[11.5px] text-muted-foreground">
                    ...
                    variant shares these photos, and each one can choose which
                    photo it opens on.
                </span>
            </div>
            <input
            ...
            <FieldHelp className="mt-2" tone={limit.full ? "warn" : "quiet"}>
                {limit.text}
            </FieldHelp>
            {busy ? (
                <div
