...
/**
 * CSV import for products (#175). Store-access gated; the api rejects writes
...
export default async function ImportProductsPage({
    ...
    const [store, descriptor] = await Promise.all([
        getStore(storeId),
        describeImport(storeId, "products"),
    ]);
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                description="Bring your catalog in from a spreadsheet."
            />
            <CsvImport
                storeId={storeId}
                descriptor={descriptor}
                backHref={`/stores/${storeId}/products`}
            />
        </div>
    );
}
