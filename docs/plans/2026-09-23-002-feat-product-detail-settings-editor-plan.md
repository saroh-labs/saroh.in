...
> **Done — #468.** `components/commerce/product-editor-v2/`: a shell (sticky header with the saved status, a hint that counts what is unsaved and what needs a fix, Save all for more than one section, section jumps with dots, a leave dialog that names the sections, the browser's own guard) and one card per section, each with its own react-hook-form or draft, Discard and Save, reporting whether it is dirty and its first problem through `useSection`. Basics (name with counter, address with the moved-links warning once it was ever live, price and MRP beside it, a searchable category picker that can create one), Description (the trimmed Tiptap editor restyled to the design's toolbar, key points with their switch), How to use and ingredients, Made by and returns (a separate "Made in" switch, supplier code team-only), Visibility (save named for the act — Publish, Take off the shop, Archive). Creating needs a name and a price; everything else is Optional and goes with the create call; photos follow once the product exists. The API now merges `shopFields` on PATCH, so sections saved in a row never undo each other (DB test added). Pure helpers — section names, Basics' problem, the header hint, Save all's partition and message — are tested. **Differences from the design, on purpose:** MRP sits beside Price with Category below it (MRP was added to scope after the design); the SEO section waits for the website work, as agreed; "More about it" waits for custom fields (#482); the ready line and allergens are the food fields (#483, #480) — for this store the same card is How to use and ingredients. The old single-form editor is removed. Verified in the browser at 1440 and 390: edit, a bad price, Save all, discard, the leave dialog, and a new product created as a draft.
...
> **Done — #469.** Photos: tiles with move earlier and later, Cover, take off with Undo and the description typed in place; the library panel toggles photos on and off; upload with progress through `useImageUpload` (pulled out of `MediaPicker`); an address fallback for when storage can't take an upload — the case on a local machine, and so for the demo films. Variants: "Customers choose by" from Settings → Options (changing it while variants exist is refused with a toast), value selects that disable taken values, suggested SKUs (`suggestSku`), optional price and photo, each row's own error, staged rows saved together (removals first, then creates and updates, then order), a failed save keeps the edits with Try again. Stock: one count or one per variant with the whole product's sum, never below what open orders promise, and a warning that names the variant. The photo library panel here and in the quick sheet no longer fetches during render (it looped). Verified in the browser: a new product, three variants with suggested SKUs, stock per variant with the 50 ml warning, three photos added and saved; the test product was then removed. **Waiting on other units:** "Manage options" links to Settings (#470); the SKU placeholder follows the store's pattern once #484 lands; "Sell it at" per variant needs more than one storefront per product and is not modelled.
...
> **Done — #482.** Tables from `20260924100000_catalogue_extras` (RLS on `ProductField` and `ProductFieldValue`). API `/stores/:id/fields` (list, add, change, soft delete, restore) in the catalogue module; product values through the section PATCH `customFields` and the product read's `customFields` — both plain functions (`productFieldsFor`, `saveProductFieldValues`) so `ProductsService` keeps its constructor. Rules pure (`field-rules.ts`, 5 tests) and 4 DB tests (asked by category, typed values, out of a category keeps values, delete + restore with values). App: the Custom fields tab, the editor's "More about it" in Made by (saved with that section; a date picker and Yes / No), a row per field on the product page, and the shop's details through a new `extras` prop on site-blocks' `ProductPage` (test added). Seeded on Leela & Loom: Skin type and Fabric care on the shop, Supplier batch team only. Verified in the browser: the tab, "More about it" on the Vitamin C serum, its rows on the product page, and the preview showing Skin type but not the batch. **Not done:** the daily purge of values 30 days after a delete — values are kept, and a purge job is left for later.
