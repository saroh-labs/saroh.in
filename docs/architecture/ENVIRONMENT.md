...
| CORS/links           | `CORS_ORIGIN`, `APP_URL`, `RENDERER_URL`, `ACCOUNTS_URL`                                                                                                                           | CORS uses the trusted-origins list + localhost; links use `https://app.saroh.in`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Trusted proxies      | `TRUST_PROXY`                                                                                                                                                                      | `cloudflare` (default): Cloudflare's edge ranges plus the private network (Traefik on Coolify, portless locally) may name the client in `X-Forwarded-For`; `@Ip()` and every rate limiter read the first address that is not one of them, so a caller who skips Cloudflare cannot pose as anyone. `private`: a proxy in front, no CDN. `none`: reached directly (a bare port, CI e2e — what an unset value means under `SKIP_ENV_VALIDATION`). The Cloudflare ranges live in `src/common/trust-proxy.ts` (from cloudflare.com/ips); update them if Cloudflare adds one. |
