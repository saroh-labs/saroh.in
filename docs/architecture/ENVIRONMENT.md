...
| Proxy hops           | `TRUST_PROXY_HOPS`                                                                                                                                                                 | Default `1`: one reverse proxy in front appends to `X-Forwarded-For` (Traefik on Coolify, portless locally) and `@Ip()` and every rate limiter read the client one hop back. `0` when the API is reached directly (a bare port, CI e2e — the default under `SKIP_ENV_VALIDATION`); one more per extra proxy, such as a proxying CDN. |
