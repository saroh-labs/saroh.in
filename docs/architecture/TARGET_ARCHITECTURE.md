...
| Membership/RBAC   | Organization invitations (`OrganizationInvitation`, hashed token, seven-day expiry) and OWNER/ADMIN/MEMBER/REVIEWER roles; per-site `SiteReviewer` grants; Teams; direct/Team Project access with MANAGER/EDITOR/VIEWER roles | centralized `authorize(actor, action, resource)`; OWNER/ADMIN see every Project                       |
...
- API resolves the authenticated actor, validates Organization membership, selects an active Organization context, then authorizes each command/query. OWNER and ADMIN see every Project; MEMBER Project access is direct or Team-derived. REVIEWER sees no Project and only the sites it was granted (#276) — any other site is a 404.
