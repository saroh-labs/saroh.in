...
| [frontend-design-system.md](frontend-design-system.md)               | Styling, tokens, icons, motion, or anything drawn on a merchant's page                         |
...
| [backend-data-and-money.md](backend-data-and-money.md)               | Changing the schema, adding a tenant-owned model, or storing money                             |
...
- **AI feature patterns.** DEC-015 plans an AI provider abstraction, but no LLM
  code exists yet. Read claude-patterns `ai-features/` when it does.
- **The `modules/<feature>/` frontend layout and React Query.** This repo is
  Server-Component-first; see `frontend-app-structure.md`.

## Keeping these true

Change a pattern file in the same commit as the code it describes, and add a
row here and in AGENTS.md → Triggers for a new one. A pattern that describes
last quarter's code is worse than none.
